...
const DieShape = ({ sides, color, value }) => {
  ...
  return (
    <div className="relative flex items-center justify-center w-8 h-8 transition-transform hover:scale-110">
      ...
      <span className="absolute inset-0 flex items-center justify-center font-bold text-white text-xs drop-shadow-md z-10">{value}</span>
    </div>
  );
};
...
function RollHistory({ logs, onClose, onClear }) {
  ...
  const [position, setPosition, keepInBounds] = useWindowPosition('vtt-history-pos', initialPos, isMobile ? { w: 280, h: 400 } : size);
  ...
  return (
    ...
    >
      <div onMouseDown={handleStart} onTouchStart={handleStart} className="bg-neutral-100 dark:bg-white/5 p-3 cursor-grab active:cursor-grabbing flex justify-between items-center border-b border-neutral-200 dark:border-white/10">
      ...
      {!isMinimized && (
        <div className="flex-1 overflow-y-auto p-3 space-y-3 relative bg-neutral-50/50 dark:bg-transparent custom-scrollbar">
          {logs.length === 0 ? <div className="flex flex-col items-center justify-center h-full text-neutral-400 opacity-50"><p className="text-xs italic">Vacío</p></div> : 
            logs.map((log) => {
              // Calculamos el total seguro (por si es un log antiguo)
              const hasModifier = log.modifier !== undefined && log.modifier !== 0;
              const displayTotal = log.total !== undefined ? log.total : log.results.reduce((a,c)=>a+c.value, 0);
              const natural = displayTotal - (log.modifier || 0);
                    ...
                    <div className="text-right leading-none">
                      ...
                      {hasModifier && (
                        <div className="text-[9px] text-neutral-400 font-mono mt-0.5">
                          {natural} {log.modifier > 0 ? '+' : ''}{log.modifier}
                        </div>
                      )}
                    </div>
                  </div>
          ...
          {!isMobile && <div onMouseDown={(e) => { e.stopPropagation(); setIsResizing(true); startSize.current = { w: size.w, h: size.h, x: e.clientX, y: e.clientY }; }} className="absolute bottom-0 right-0 w-6 h-6 cursor-nwse-resize p-1 opacity-30 hover:opacity-100"><div className="w-2 h-2 border-r-2 border-b-2 border-neutral-400 dark:border-white/50"></div></div>}
        </div>
      )}
    </div>
  );
