...
function LibraryManager({ library, connectedPlayers, currentUser, onEmitResource, onUpdateResource, onDeleteResource, onOpenResource, onClose }) {
  ...
  useEffect(() => {
    ...
    let changed = false;
    usedFolders.forEach(f => { if (!updated.includes(f)) { updated.push(f); changed = true; } });
    if (changed) { setCustomFolders(updated); localStorage.setItem('vtt-custom-folders', JSON.stringify(updated)); }
  }, [library]);
  ...
  // Jugadores conocidos (Conectados + Históricos, sin GM)
  ...
  const handleFileSelect = (e) => {
    ...
    if (!file) return;
    if (file.size > 2 * 1024 * 1024) return alert("⚠️ Archivo demasiado grande (>2MB).");
  ...
  const toggleAllowedPlayer = (playerName) => {
    setFormData(prev => {
      ...
      // Si hay jugadores en la lista, visibilidad es 'specific'. Si no, vuelve a 'hidden' por seguridad.
      const newVisibility = newList.length > 0 ? 'specific' : 'hidden';
      return { ...prev, allowedPlayers: newList, visibility: newVisibility };
    });
  };
  ...
  // --- ACCIONES ---
  const handleCreateNew = () => { setEditingId(null); setFormData({ type: 'text', title: '', content: '', folder: selectedFolder === 'Todas' ? 'General' : selectedFolder, visibility: 'hidden', allowedPlayers: [], attachment: '' }); setView('editor'); };
  const handleEdit = (item) => { if (item.author !== currentUser.name && !currentUser.isGM) return; setEditingId(item.id); setFormData({ type: item.type, title: item.title, content: item.content, folder: item.folder, visibility: item.visibility, allowedPlayers: item.allowedPlayers || [], attachment: item.attachment || '' }); setView('editor'); };
          ...
          {view === 'editor' && (
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-neutral-50 dark:bg-black/20 cursor-default">
              <form onSubmit={handleSave} className="max-w-3xl mx-auto space-y-6">
                ...
                {/* 2. TIPO DE RECURSO (Bloque Vertical 1) */}
                <div className="bg-white dark:bg-white/5 rounded-xl border border-neutral-200 dark:border-white/10 p-4">
                   ...
                   <div className="flex gap-2">
                      ...
                      <button type="button" disabled className="flex-1 py-3 rounded-lg text-sm font-bold border border-neutral-100 dark:border-white/5 text-neutral-300 dark:text-neutral-700 flex flex-col items-center gap-1 cursor-not-allowed bg-neutral-50 dark:bg-black/20">
                        <Music size={20}/> Música
                      </button>
                   </div>
                </div>

                {/* 3. VISIBILIDAD (Bloque Vertical 2) */}
                <div className="bg-white dark:bg-white/5 rounded-xl border border-neutral-200 dark:border-white/10 p-4">
                  <label className="block text-xs font-bold text-neutral-500 uppercase mb-3">Visibilidad</label>
                  <div className="flex flex-col gap-3">
                    {/* Opciones Globales */}
                    <div className="flex gap-2">
                       <button type="button" onClick={() => setGlobalVisibility('hidden')} className={`flex-1 py-2 rounded-lg text-sm font-bold border flex items-center justify-center gap-2 ${formData.visibility === 'hidden' ? 'bg-red-100 border-red-300 text-red-800' : 'bg-transparent border-neutral-200 dark:border-white/10 text-neutral-500'}`}>
                         <Lock size={16}/> Oculto
                       </button>
                       <button type="button" onClick={() => setGlobalVisibility('visible')} className={`flex-1 py-2 rounded-lg text-sm font-bold border flex items-center justify-center gap-2 ${formData.visibility === 'visible' ? 'bg-emerald-100 border-emerald-300 text-emerald-800' : 'bg-transparent border-neutral-200 dark:border-white/10 text-neutral-500'}`}>
                         <Eye size={16}/> Público (Todos)
                       </button>
                    </div>

                    {/* Lista de Jugadores Específicos */}
                    <div className="border-t border-neutral-200 dark:border-white/10 pt-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {allKnownPlayers.length === 0 && <span className="text-xs text-neutral-400 italic">No hay otros jugadores registrados.</span>}
                        {allKnownPlayers.map(player => {
                          const isSelected = formData.allowedPlayers?.includes(player);
                          return (
                            <button 
                              key={player} 
                              type="button" 
                              onClick={() => toggleAllowedPlayer(player)}
                              className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-colors ${isSelected ? 'bg-blue-100 border-blue-300 text-blue-700' : 'bg-neutral-50 dark:bg-white/5 border-neutral-200 dark:border-white/10 text-neutral-500'}`}
                            >
                              {player}
                            </button>
                          );
                ...
                {(formData.type === 'image' || formData.type === 'pdf') && (
                    <div>
                        ...
                        {formData.content && formData.type === 'pdf' && (
                            <div className="mt-4 border-2 border-dashed border-neutral-300 dark:border-white/10 rounded-xl p-8 flex flex-col items-center justify-center bg-neutral-100 dark:bg-white/5 text-neutral-500">
                                <File size={48} className="mb-2 text-red-400"/>
                                <span className="text-sm font-bold">PDF Vinculado Correctamente</span>
                                <span className="text-xs">Visualízalo pulsando "Mostrar" en la lista.</span>
                            </div>
                        )}
                    </div>
                )}
