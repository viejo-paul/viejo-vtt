...
import { 
  ...
  ChevronUp, ChevronDown, ChevronRight, CornerDownRight, Music, File, Users 
} from 'lucide-react';
import { usePersistentState } from '../hooks/usePersistentState';
import { useWindowPosition } from '../hooks/useWindowPosition'; // Nos aseguramos de importarlo

function LibraryManager({ library, connectedPlayers, currentUser, onEmitResource, onUpdateResource, onDeleteResource, onOpenResource, openResources = [], onClose, zIndex, onFocus }) {
  const [view, setView] = useState('list');
  const [selectedFolder, setSelectedFolder] = useState('Todas');
  const [searchTerm, setSearchTerm] = useState('');
  
  // 1. POSICIÓN Y TAMAÑO
  // Usamos useWindowPosition para la posición inicial y su lógica de límites
  const [position, setPosition, keepInBounds] = useWindowPosition('vtt-library-pos', { x: 80, y: 60 });
  const [size, setSize] = usePersistentState('vtt-library-size', { w: 900, h: 600 });

  // 2. ESTADOS DE ARRASTRE
  ...
  // INICIO ARRASTRE (Solo desde la barra superior)
  const handleStartDrag = (e) => {
    if (isResizing) return;
    // Evitamos arrastrar si tocamos un botón o input dentro del header
    if (e.target.closest('button') || e.target.closest('input')) return;
    
    e.preventDefault(); // Importante para evitar selección de texto
  ...
  // MOVIMIENTO GLOBAL (Drag y Resize)
  useEffect(() => {
    const handleMove = (e) => {
      const clientX = e.touches ? e.touches[0].clientX : e.clientX;
      const clientY = e.touches ? e.touches[0].clientY : e.clientY;
      ...
      if (isDragging) {
        // Usamos keepInBounds si está disponible, sino cálculo directo
        const newX = clientX - dragOffset.x;
        const newY = clientY - dragOffset.y;
        setPosition(keepInBounds ? keepInBounds(newX, newY) : { x: newX, y: newY });
      }
      ...
      if (isResizing) {
        ...
        if (dir.includes('e')) newW = Math.max(400, startResize.current.w + deltaX);
        if (dir.includes('s')) newH = Math.max(300, startResize.current.h + deltaY);
        if (dir.includes('w')) {
          const possibleW = Math.max(400, startResize.current.w - deltaX);
          if (possibleW !== 400) { newW = possibleW; newX = startResize.current.posX + deltaX; }
        }
        if (dir.includes('n')) {
          const possibleH = Math.max(300, startResize.current.h - deltaY);
          if (possibleH !== 300) { newH = possibleH; newY = startResize.current.posY + deltaY; }
        }
  ...
  }, [isDragging, isResizing, dragOffset]); // Dependencias corregidas

  // --- LÓGICA DE NEGOCIO (Igual que antes) ---
  const [customFolders, setCustomFolders] = useState(() => { const saved = localStorage.getItem('vtt-custom-folders'); return saved ? JSON.parse(saved) : ['General', 'Mapas', 'PNJs', 'Pistas', 'Reglas']; });
  ...
  const itemsByFolder = useMemo(() => { const groups = {}; library.forEach(item => { if (!groups[item.folder]) groups[item.folder] = []; groups[item.folder].push(item); }); return groups; }, [library]);
  ...
  const [formData, setFormData] = useState({ type: 'text', title: '', content: '', folder: 'General', visibility: 'hidden', allowedPlayers: [], attachment: '', colorId: 'default', isFrameless: false });
  const fileInputRef = useRef(null);
  const [uploadTarget, setUploadTarget] = useState(null); 
  const allKnownPlayers = useMemo(() => { const currentNames = connectedPlayers.map(p => p.name); const authorNames = library.map(i => i.author); return [...new Set([...currentNames, ...authorNames])].filter(name => name !== 'GM' && name !== 'DJ' && name !== 'Narrador'); }, [connectedPlayers, library]);
  const handleTriggerUpload = (target) => { setUploadTarget(target); fileInputRef.current.click(); };
  const handleFileSelect = (e) => { const file = e.target.files[0]; if (!file) return; if (file.size > 5 * 1024 * 1024) return alert("⚠️ Archivo demasiado grande (>5MB)."); const reader = new FileReader(); reader.onload = (ev) => setFormData(prev => ({ ...prev, [uploadTarget]: ev.target.result })); reader.readAsDataURL(file); };
  const toggleAllowedPlayer = (playerName) => { setFormData(prev => { const currentList = prev.allowedPlayers || []; const newList = currentList.includes(playerName) ? currentList.filter(n => n !== playerName) : [...currentList, playerName]; return { ...prev, allowedPlayers: newList, visibility: newList.length > 0 ? 'specific' : 'hidden' }; }); };
  const setGlobalVisibility = (mode) => setFormData(prev => ({ ...prev, visibility: mode, allowedPlayers: [] }));
  const filteredItems = library.filter(item => { if (selectedFolder !== 'Todas' && item.folder !== selectedFolder) return false; if (searchTerm && !item.title.toLowerCase().includes(searchTerm.toLowerCase())) return false; const isAuthor = item.author === currentUser.name; const isGM = currentUser.isGM; const isAllowed = item.allowedPlayers?.includes(currentUser.name); if (isGM || isAuthor || item.visibility === 'visible' || item.visibility === 'public' || isAllowed) return true; return false; });
  ...
  return (
    <div 
      ...
      className="fixed bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl overflow-hidden flex flex-col border border-neutral-200 dark:border-white/10 animate-in fade-in zoom-in duration-200"
    >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-72 bg-neutral-100 dark:bg-black/40 border-r border-neutral-200 dark:border-white/10 flex flex-col hidden md:flex no-drag">
           <div 
             ...
             className="p-4 border-b border-neutral-200 dark:border-white/10 flex justify-between items-center cursor-move select-none"
                ...
                {filteredItems.map(item => (
                  <div key={item.id} draggable="true" onDragStart={(e) => handleItemDragStart(e, item.id)} className="group relative bg-white dark:bg-white/5 border border-neutral-200 dark:border-white/10 rounded-xl p-4 hover:border-emerald-500 dark:hover:border-emerald-500 transition-all shadow-sm hover:shadow-md flex flex-col">
                    ...
                    <div className="flex gap-3 mt-auto">
                      <button onClick={() => onOpenResource(item)} className={`flex-1 py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all ${openResources.find(r => r.id === item.id) ? 'bg-neutral-800 text-neutral-400 hover:bg-red-600 hover:text-white dark:bg-white/20' : 'bg-neutral-100 dark:bg-white/10 hover:bg-emerald-600 hover:text-white dark:hover:bg-emerald-600'}`}>
                        {openResources.find(r => r.id === item.id) ? (<><Eye size={16} className="opacity-50"/> OCULTAR</>) : (<><Eye size={16}/> MOSTRAR</>)}
                      </button>
          ...
          {view === 'editor' && (
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-neutral-50 dark:bg-black/20 cursor-default">
              <form onSubmit={handleSave} className="max-w-3xl mx-auto space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-xs font-bold text-neutral-500 uppercase mb-1">Título</label><input autoFocus type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full p-2 bg-white dark:bg-black/40 border border-neutral-300 dark:border-white/20 rounded-lg outline-none focus:ring-2 focus:ring-emerald-500 dark:text-white" required /></div><div><label className="block text-xs font-bold text-neutral-500 uppercase mb-1">Carpeta</label><select value={formData.folder} onChange={e => setFormData({...formData, folder: e.target.value})} className="w-full p-2 bg-white dark:bg-black/40 border border-neutral-300 dark:border-white/20 rounded-lg outline-none focus:ring-2 focus:ring-emerald-500 dark:text-white">{customFolders.map(f => <option key={f} value={f}>{f}</option>)}</select></div></div><div className="bg-white dark:bg-white/5 rounded-xl border border-neutral-200 dark:border-white/10 p-4"><label className="block text-xs font-bold text-neutral-500 uppercase mb-2">Tipo de Recurso</label><div className="flex gap-2"><button type="button" onClick={() => setFormData({...formData, type: 'text'})} className={`flex-1 py-3 rounded-lg text-sm font-bold border flex flex-col items-center gap-1 transition-all ${formData.type === 'text' ? 'bg-yellow-100 border-yellow-300 text-yellow-800' : 'bg-transparent border-neutral-200 dark:border-white/10 text-neutral-500 hover:bg-black/5 dark:hover:bg-white/5'}`}><FileText size={20}/> Texto</button><button type="button" onClick={() => setFormData({...formData, type: 'image'})} className={`flex-1 py-3 rounded-lg text-sm font-bold border flex flex-col items-center gap-1 transition-all ${formData.type === 'image' ? 'bg-blue-100 border-blue-300 text-blue-800' : 'bg-transparent border-neutral-200 dark:border-white/10 text-neutral-500 hover:bg-black/5 dark:hover:bg-white/5'}`}><ImageIcon size={20}/> Imagen</button><button type="button" onClick={() => setFormData({...formData, type: 'pdf'})} className={`flex-1 py-3 rounded-lg text-sm font-bold border flex flex-col items-center gap-1 transition-all ${formData.type === 'pdf' ? 'bg-red-100 border-red-300 text-red-800' : 'bg-transparent border-neutral-200 dark:border-white/10 text-neutral-500 hover:bg-black/5 dark:hover:bg-white/5'}`}><File size={20}/> PDF</button><button type="button" disabled className="flex-1 py-3 rounded-lg text-sm font-bold border border-neutral-100 dark:border-white/5 text-neutral-300 dark:text-neutral-700 flex flex-col items-center gap-1 cursor-not-allowed bg-neutral-50 dark:bg-black/20"><Music size={20}/> Música</button></div>{formData.type === 'text' && (<div className="mt-4 pt-4 border-t border-neutral-200 dark:border-white/10"><label className="block text-xs font-bold text-neutral-500 uppercase mb-2">Color de la Nota</label><div className="flex gap-2">{[{ id: 'default', bg: 'bg-white dark:bg-neutral-800' },{ id: 'yellow', bg: 'bg-yellow-50 dark:bg-yellow-900/20' },{ id: 'red', bg: 'bg-red-50 dark:bg-red-900/20' },{ id: 'blue', bg: 'bg-blue-50 dark:bg-blue-900/20' },{ id: 'green', bg: 'bg-emerald-50 dark:bg-emerald-900/20' },{ id: 'purple', bg: 'bg-purple-50 dark:bg-purple-900/20' },{ id: 'dark', bg: 'bg-neutral-900 text-white' }].map(c => (<button key={c.id} type="button" onClick={() => setFormData({...formData, colorId: c.id})} className={`w-8 h-8 rounded-full border-2 ${c.bg} ${formData.colorId === c.id ? 'border-emerald-500 scale-110 shadow' : 'border-transparent opacity-50 hover:opacity-100'} transition-all`}/>))}</div></div>)}{formData.type === 'image' && (<div className="mt-4 pt-4 border-t border-neutral-200 dark:border-white/10"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={formData.isFrameless} onChange={e => setFormData({...formData, isFrameless: e.target.checked})} className="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500 bg-neutral-100 border-neutral-300 dark:bg-black/40 dark:border-white/20"/><span className="text-sm text-neutral-700 dark:text-neutral-300">Modo Ficha/Token (Sin marco ni fondo)</span></label></div>)}</div><div className="bg-white dark:bg-white/5 rounded-xl border border-neutral-200 dark:border-white/10 p-4"><label className="block text-xs font-bold text-neutral-500 uppercase mb-3">Visibilidad</label><div className="flex flex-col gap-3"><div className="flex gap-2"><button type="button" onClick={() => setGlobalVisibility('hidden')} className={`flex-1 py-2 rounded-lg text-sm font-bold border flex items-center justify-center gap-2 ${formData.visibility === 'hidden' ? 'bg-red-100 border-red-300 text-red-800' : 'bg-transparent border-neutral-200 dark:border-white/10 text-neutral-500'}`}><Lock size={16}/> Oculto</button><button type="button" onClick={() => setGlobalVisibility('visible')} className={`flex-1 py-2 rounded-lg text-sm font-bold border flex items-center justify-center gap-2 ${formData.visibility === 'visible' ? 'bg-emerald-100 border-emerald-300 text-emerald-800' : 'bg-transparent border-neutral-200 dark:border-white/10 text-neutral-500'}`}><Eye size={16}/> Público (Todos)</button></div><div className="border-t border-neutral-200 dark:border-white/10 pt-3"><p className="text-[10px] text-neutral-400 uppercase font-bold mb-2">Compartir solo con:</p><div className="flex flex-wrap gap-2">{allKnownPlayers.length === 0 && <span className="text-xs text-neutral-400 italic">No hay otros jugadores.</span>}{allKnownPlayers.map(player => (<button key={player} type="button" onClick={() => toggleAllowedPlayer(player)} className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-colors ${formData.allowedPlayers?.includes(player) ? 'bg-blue-100 border-blue-300 text-blue-700' : 'bg-neutral-50 dark:bg-white/5 border-neutral-200 dark:border-white/10 text-neutral-500'}`}>{player}</button>))}</div></div></div></div>{formData.type === 'text' ? (<div className="space-y-4"><div><label className="block text-xs font-bold text-neutral-500 uppercase mb-1">Imagen Cabecera</label><div className="flex gap-2"><input type="text" value={formData.attachment} onChange={e => setFormData({...formData, attachment: e.target.value})} className="flex-1 p-2 bg-white dark:bg-black/40 border border-neutral-300 dark:border-white/20 rounded-lg outline-none dark:text-white text-sm" placeholder="URL opcional..." /><button type="button" onClick={() => handleTriggerUpload('attachment')} className="px-3 bg-neutral-200 dark:bg-white/10 hover:bg-neutral-300 dark:hover:bg-white/20 rounded-lg transition-colors"><Upload size={18}/></button></div></div><div className="bg-white dark:bg-neutral-800 rounded-lg overflow-hidden border border-neutral-300 dark:border-white/20 text-neutral-900 dark:text-white"><ReactQuill theme="snow" value={formData.content} onChange={value => setFormData({...formData, content: value})} modules={{ toolbar: [[{ 'header': [1, 2, false] }],['bold', 'italic', 'underline'],[{ 'list': 'ordered'}, { 'list': 'bullet' }],['clean']] }} className="h-64 mb-12" /></div></div>) : (<div><label className="block text-xs font-bold text-neutral-500 uppercase mb-1">{formData.type === 'image' ? 'Imagen' : 'PDF'} (URL o Subir)</label><div className="flex gap-2"><input type="text" value={formData.content} onChange={e => setFormData({...formData, content: e.target.value})} className="flex-1 p-2 bg-white dark:bg-black/40 border border-neutral-300 dark:border-white/20 rounded-lg outline-none dark:text-white font-mono text-sm" placeholder="https://..." /><button type="button" onClick={() => handleTriggerUpload('content')} className="px-3 bg-neutral-200 dark:bg-white/10 hover:bg-neutral-300 dark:hover:bg-white/20 rounded-lg transition-colors" title="Subir"><Upload size={18}/></button></div>{formData.content && formData.type === 'image' && <div className="mt-4 border-2 border-dashed border-neutral-300 dark:border-white/10 rounded-xl p-4 flex justify-center bg-black/50"><img src={formData.content} alt="Preview" className="max-h-64 object-contain shadow-lg" /></div>}{formData.content && formData.type === 'pdf' && <div className="mt-4 border-2 border-dashed border-neutral-300 dark:border-white/10 rounded-xl p-8 flex flex-col items-center justify-center bg-neutral-100 dark:bg-white/5 text-neutral-500"><File size={48} className="mb-2 text-red-400"/><span className="text-sm font-bold">PDF Vinculado Correctamente</span></div>}</div>)}<div className="flex gap-4 pt-4 border-t border-neutral-200 dark:border-white/10">{editingId && <button type="button" onClick={handleDelete} className="px-4 py-3 bg-red-100 hover:bg-red-200 text-red-700 rounded-xl font-bold flex items-center gap-2"><Trash2 size={18} /> Borrar</button>}<div className="flex-1"></div><button type="submit" className="px-8 py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold shadow-lg flex items-center gap-2"><Save size={18} /> Guardar</button></div>
              </form>
            </div>
          )}
        </div>
