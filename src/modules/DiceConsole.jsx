...
function DiceConsole({ onClose, onRoll }) {
  ...
  const [diceCount, setDiceCount] = usePersistentState('vtt-console-count', 1);
  const [diceColor, setDiceColor] = usePersistentState('vtt-console-color', '#e5e5e5');
  ...
  const addSelectionToPool = () => {
    ...
    setDicePool([...dicePool, { 
      ...
      sides: selectedDie, 
      qty: diceCount, 
      color: diceColor // Guardamos el color actual aquí
    }]);
  };
  ...
  const triggerRoll = () => {
    ...
    const rollConfig = dicePool.map(group => ({
      ...
      qty: group.qty,
      themeColor: group.color // Enviamos el color guardado de cada grupo
    }));
    onRoll(rollConfig);
  };
    ...
    >
      ...
      <div onMouseDown={handleMouseDown} onTouchStart={handleTouchStart} className="p-3 cursor-grab active:cursor-grabbing flex justify-between items-center bg-neutral-100 dark:bg-white/5 border-b border-neutral-200 dark:border-white/10">
        ...
        <div className="flex items-center gap-1">
          <button type="button" onClick={() => setIsMinimized(!isMinimized)} className="text-neutral-500 hover:text-black dark:hover:text-white p-1"><Minus size={16} /></button>
          <button type="button" onClick={onClose} className="text-neutral-500 hover:text-red-500 p-1"><X size={16} /></button>
        </div>
      </div>
        ...
        <>
            ...
            <div className="p-3 border-b border-neutral-200 dark:border-white/10 space-y-3 bg-white dark:bg-neutral-800/20">
              ...
              <div className="px-1">
                ...
                <div className="flex flex-wrap justify-between gap-2 items-center">
                  {colors.map((c) => (
                    <button key={c.name} type="button" onClick={() => setDiceColor(c.hex)} className={`w-5 h-5 rounded-full border transition-all ${diceColor === c.hex ? 'ring-2 ring-emerald-500 scale-110 shadow-md border-white' : 'opacity-70 border-neutral-300 dark:border-white/20'}`} style={{ backgroundColor: c.hex }} />
                  ))}
                  <div className="relative w-5 h-5">
                    <div className={`w-5 h-5 rounded-full bg-gradient-to-tr from-blue-400 to-red-500 flex items-center justify-center ${isCustomColor ? 'ring-2 ring-emerald-500 scale-110 shadow-lg' : 'opacity-70'}`}>
                      ...
                      <input type="color" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-20" value={diceColor} onChange={(e) => setDiceColor(e.target.value)} />
                    </div>
                  </div>
                </div>
              </div>
          ...
          {dicePool.length > 0 && (
            <div className="p-2 bg-neutral-100 dark:bg-emerald-900/10">
              ...
              <button type="button" onClick={triggerRoll} className="w-full bg-emerald-600 py-3 rounded-xl font-bold text-sm text-white shadow-lg active:scale-95 flex justify-center items-center gap-2"><Dices size={18} /> LANZAR</button>
            </div>
          )}
        </>
      )}
